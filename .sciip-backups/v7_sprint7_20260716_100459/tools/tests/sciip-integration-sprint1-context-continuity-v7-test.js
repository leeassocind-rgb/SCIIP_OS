...
if(!failures.length){
 ...
 ['SCIIP_CONTEXT_CONTINUITY','delete copy.meta','function sciipContextBackV7','function sciipContextForwardV7','function sciipContextDeepLinkV7','MAX_HISTORY = 50'].forEach(x=>{if(!src.includes(x))failures.push('Missing contract '+x)});
 const tests=read('src/ui/SCIIP_Integration_Tests.gs');
 ['before.count!==3','sciipTestV7ContextHistory','sciipTestV7ContextDeepLinkRoundTrip','sciipTestV7ContextRestore','sciipTestV7IntegrationSprint1ContextContinuity'].forEach(x=>{if(!tests.includes(x))failures.push('Missing test '+x)});
