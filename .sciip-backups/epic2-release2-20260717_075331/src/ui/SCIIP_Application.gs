...
var SCIIP_APPLICATION = (function () {
  'use strict';
  var VERSION = 'v7.0-epic2-release1.0';
  var WORKSPACES = [
  ...
  function dataSources_(){
    ...
    return {
      ...
      historicalBacklog:{status:'READY',label:'Supersheets saved since June',queueMode:'DRIVE_FOLDER_OR_FILE_IDS',batchSafe:true},
      actions:['UPLOAD_SOURCE','REGISTER_DRIVE_FOLDER','CREATE_IMPORT_JOB','REVIEW_RECORDS','PREPARE_COMMIT','EXECUTE_COMMIT','ROLLBACK']
    };
  }
...
/** Register historical Supersheets from a Drive folder without importing them yet. */
function sciipRegisterHistoricalSupersheetFolder(folderId){
  if(!folderId)throw new Error('folderId is required.');
  var folder=DriveApp.getFolderById(folderId),files=folder.getFiles(),items=[];
  while(files.hasNext()){var f=files.next(),mime=f.getMimeType();if(/spreadsheet|excel|csv|sheet/i.test(mime+' '+f.getName()))items.push({fileId:f.getId(),name:f.getName(),mimeType:mime,url:f.getUrl(),modifiedAt:f.getLastUpdated().toISOString(),status:'QUEUED_FOR_RECOGNITION'});}
  return {status:'REGISTERED',folderId:folderId,folderName:folder.getName(),files:items.length,items:items};
}
