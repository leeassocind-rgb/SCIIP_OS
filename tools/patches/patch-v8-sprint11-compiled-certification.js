...
const fn="sciipTestV8Sprint11EnterpriseCollaborationNotificationsCaseManagement";
...
if(found){console.log("Sprint 11 test already compiled in "+path.relative(repo,found));process.exit(0);}
...
const wrapper=`
/** SCIIP_OS v8.0 Sprint 11 explicit compiled certification wrapper. */
function ${fn}(){
  if(typeof SCIIP_V8_COLLABORATION_CASES==="undefined"||!SCIIP_V8_COLLABORATION_CASES||typeof SCIIP_V8_COLLABORATION_CASES.certify!=="function"){
    throw new Error("SCIIP V8 Sprint 11 collaboration application is unavailable.");
  }
  var result=SCIIP_V8_COLLABORATION_CASES.certify();
  if(!result||result.status!=="PASSED"){throw new Error("SCIIP V8 Sprint 11 certification failed: "+JSON.stringify(result));}
