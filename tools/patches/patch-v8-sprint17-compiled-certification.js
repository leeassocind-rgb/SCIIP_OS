...
if(found){console.log("Sprint 17 certification already compiled in "+path.relative(repo,found));process.exit(0);}
const source=path.join(repo,"src","applications","enterprise-digital-twin-simulation","SCIIP_V8_Enterprise_Digital_Twin_Simulation_Predictive_Operations.gs");
if(!fs.existsSync(source)){console.error("Missing Sprint 17 source application.");process.exit(1);}
...
const code=fs.readFileSync(source,"utf8"),start=code.indexOf("var SCIIP_V8_ENTERPRISE_DIGITAL_TWIN_SIMULATION=");
if(start<0){console.error("Sprint 17 namespace not found in source.");process.exit(1);}
fs.appendFileSync(target,"\n\n/** SCIIP_OS v8.0 Sprint 17 explicit compiled application and certification wrapper. */\n"+code.slice(start)+"\n");
