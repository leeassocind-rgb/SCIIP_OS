...
for(const f of files){
  ...
  for(const name of topLevelFunctionNames(src)){if(!funcs.has(name))funcs.set(name,[]);funcs.get(name).push(r)}
  for(const declaration of topLevelVarDeclarations(src)){if(!globals.has(declaration.name))globals.set(declaration.name,[]);globals.get(declaration.name).push({file:r,expression:declaration.expression})}
  ...
  if(r.startsWith("src/processors/runtime/storage/")&&/Processor\.gs$/.test(base)){metrics.storageProcessorFiles++;if(/SCIIP_RUNTIME_PROCESSOR_BASE|SCIIP_RUNTIME\.(PROCESSOR_BASE|runProcessor|executeProcessor)|\bSCIIP_[A-Z0-9_]+_(?:BACKEND|EXECUTION)\.execute[A-Za-z0-9_]*Plan\s*\(/.test(src))metrics.storageRuntimeBaseFiles++;if(/\b(setValue|setValues|appendRow|insertRows|deleteRows|clearContent)\s*\(/.test(clean))metrics.storageWriteFiles++}
...
fs.writeFileSync(SUMMARY,md+"\n");console.log(JSON.stringify(metrics,null,2));if(errors.length){console.error(`ARCHITECTURE GOVERNANCE FAILED: ${errors.length} error finding(s)`);errors.forEach(function(e){var locations=(e.locations||[e.file]).filter(Boolean);console.error(`- ${e.type}: ${e.name||e.message||"unnamed"}${locations.length?" -> "+locations.join(", "):""}`);});process.exit(1)}console.log("ARCHITECTURE GOVERNANCE PASSED");
