...
const uiEvidence=(()=>{try{return JSON.parse(fs.readFileSync(path.join(ROOT,"governance/ui-certification.json"),"utf8"));}catch(_){return null;}})();
domains.ui=domain("UI",[
 {name:"ui-assets",ok:html.length>=3,detail:`${html.length} HTML files`},
 {name:"ui-entrypoint",ok:exists("src/ui/SCIIP_UI_Foundation.gs")&&contains(src,/HtmlService\.createTemplateFromFile/)},
 {name:"ui-shell",ok:exists("src/ui/SCIIP_UI_Shell.html")},
 {name:"ui-evidence",ok:!!uiEvidence&&uiEvidence.status==="PASSED",detail:uiEvidence?uiEvidence.certificateId:"missing"}
]);

const apiEvidence=(()=>{try{return JSON.parse(fs.readFileSync(path.join(ROOT,"governance/api-certification.json"),"utf8"));}catch(_){return null;}})();
domains.api=domain("API",[
 {name:"api-entrypoints",ok:contains(src,/function\s+doGet\s*\(/)&&contains(src,/function\s+doPost\s*\(/)},
 {name:"api-contract",ok:exists("governance/api-contract-v1.json")&&contains(src,/API_VERSION|apiVersion/)},
 {name:"api-router",ok:contains(src,/SCIIP_API/)&&contains(src,/dispatch/)},
 {name:"api-auth-fail-closed",ok:contains(src,/AUTH_NOT_CONFIGURED/)},
 {name:"api-evidence",ok:!!apiEvidence&&apiEvidence.status==="PASSED",detail:apiEvidence?apiEvidence.certificateId:"missing"}
]);
let secretFindings=[];
...
const recoveryEvidence=(()=>{try{return JSON.parse(fs.readFileSync(path.join(ROOT,"governance/recovery-certification.json"),"utf8"));}catch(_){return null;}})();
domains.recovery=domain("Recovery",[
 {name:"checkpoint-script",ok:exists("scripts/sciip_repository_checkpoint.sh")},
 {name:"restore-script",ok:exists("scripts/sciip_repository_restore.sh")},
 ...
 {name:"recovery-evidence",ok:!!recoveryEvidence&&recoveryEvidence.status==="PASSED",detail:recoveryEvidence?recoveryEvidence.certificateId:"missing"}
]);
