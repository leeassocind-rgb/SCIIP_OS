...
const checks={files:missing.length===0,version:/v7\.0-alpha\.1/.test(dashboard),snapshot:/function snapshot\(\)/.test(dashboard),fourKpis:/kpis:\[/.test(dashboard),certifiedDomains:/CERTIFIED_DOMAINS/.test(dashboard)&&/Knowledge Graph/.test(dashboard),serviceBinding:/SCIIP_SERVICE_CONTAINER/.test(dashboard),storageBinding:/SCIIP_STORAGE_BACKEND/.test(dashboard),apiBinding:/SCIIP_API/.test(dashboard),desktopBinding:/executiveDashboard:/.test(desktop),clientRenderer:/renderExecutiveDashboard/.test(app),responsive:/dashboard-kpi-grid/.test(styles)};
