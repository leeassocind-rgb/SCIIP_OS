...
const claspSrc=path.join(ROOT,'.clasp.json'); if(fs.existsSync(claspSrc)){const c=JSON.parse(fs.readFileSync(claspSrc));c.rootDir=POLICY.outputDir;fs.writeFileSync(path.join(OUT,'.clasp.json'),JSON.stringify(c,null,2)+'\n');}
