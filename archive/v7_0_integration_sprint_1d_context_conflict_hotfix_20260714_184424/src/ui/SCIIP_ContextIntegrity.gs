...
var SCIIP_CONTEXT_INTEGRITY = (function () {
  ...
  var VERSION = 'v7.0-integration-sprint-1d';
  var MAX_AUDIT = 100;
  var audit = [];
  var resolvers = {};
  ...
  function detectConflict(baseSnapshot,incomingPatch,currentSnapshot) {
    ...
    Object.keys(incomingPatch).forEach(function(key){var base=JSON.stringify(canonical_(baseSnapshot[key])), current=JSON.stringify(canonical_(currentSnapshot[key])), incoming=JSON.stringify(canonical_(incomingPatch[key]));if(base!==current&&current!==incoming)conflicts.push({key:key,base:clone_(baseSnapshot[key]),current:clone_(currentSnapshot[key]),incoming:clone_(incomingPatch[key])});});
