 ...
 *
 * Compatibility-first execution facade for storage processors. Services may be
 * injected for testing or future backends; the default control-plane behavior
 * remains capacity safe and write free.
 */
var SCIIP_STORAGE_RUNTIME = (function () {
  ...
  function requireConfig(cfg) {
    ...
    required.forEach(function (key) {
      if (cfg[key] === undefined || cfg[key] === null || cfg[key] === '') {
        throw new Error('SCIIP_STORAGE_RUNTIME missing required configuration: ' + key);
      }
    });
    return cfg;
  }
  ...
  function dateKey() {
    var now = new Date();
    if (services.DateService && typeof services.DateService.dateKey === 'function') {
      return services.DateService.dateKey(now);
    }
  ...
  ns.getActiveSpreadsheetSafe = function () {
    if (services.StorageService && typeof services.StorageService.getActiveSpreadsheetSafe === 'function') {
      return services.StorageService.getActiveSpreadsheetSafe();
    }
    try { return SpreadsheetApp.getActiveSpreadsheet() || null; } catch (err) { return null; }
  };

  ns.getWorkbookCellCount = function (ss) {
    if (services.StorageService && typeof services.StorageService.getWorkbookCellCount === 'function') {
      return services.StorageService.getWorkbookCellCount(ss);
    }
  ...
  ns.buildBusinessKey = function (cfg) {
    requireConfig(cfg);
    if (services.TransactionService && typeof services.TransactionService.buildBusinessKey === 'function') {
      return services.TransactionService.buildBusinessKey(cfg, dateKey());
    }
  ...
  ns.buildTransactionId = function (cfg) {
    requireConfig(cfg);
    if (services.TransactionService && typeof services.TransactionService.buildTransactionId === 'function') {
      return services.TransactionService.buildTransactionId(cfg, dateKey());
    }
  ...
  ns.executeControlPlaneOnly = function (cfg) {
    requireConfig(cfg);
    if (services.StorageService && typeof services.StorageService.executeControlPlaneOnly === 'function') {
      return services.StorageService.executeControlPlaneOnly(cfg, ns);
    }
    ...
    if (services.LoggingService && typeof services.LoggingService.info === 'function') {
      services.LoggingService.info('SCIIP_STORAGE_RUNTIME_EXECUTION', result);
    }
    return result;
  };
