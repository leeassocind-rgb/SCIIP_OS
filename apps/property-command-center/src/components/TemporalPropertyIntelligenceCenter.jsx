...
import data from'../production-validation/temporal-intelligence-data.json';
const fmt=n=>new Intl.NumberFormat('en-US').format(Number(n||0));
const money=n=>n==null?'—':`$${Number(n).toFixed(2)}`;
export default function TemporalPropertyIntelligenceCenter(){
 const [type,setType]=useState('ALL');
 const events=useMemo(()=>data.events.filter(e=>type==='ALL'||e.eventType===type).slice().reverse().slice(0,250),[type]);
