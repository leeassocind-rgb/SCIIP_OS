/** Explicit Apps Script certification for SCIIP_OS v7.0 Integration Sprint 5B. */
...
function sciipTestV7OpportunityPlatformWiring(){
  ...
  if(r.status!=='WIRED')throw new Error('Sprint 5B platform wiring failed: '+JSON.stringify(r));
  return {test:'PlatformWiring',status:'PASSED',assemblyStatus:r.status,queryRegistered:r.queryRegistered,liveServiceRegistered:r.liveServiceRegistered};
}

function sciipTestV7IntegrationSprint5B(){
  var tests=[
    sciipTestV7AutonomousOpportunityEngine(),
    sciipTestV7AnomalyWatchlists(),
    sciipTestV7AutonomousActionPlanner(),
    sciipTestV7ConfidenceGovernance(),
    sciipTestV7OpportunityOrchestration(),
    sciipTestV7OpportunityPlatformWiring()
  ];
  var output={framework:'SCIIP_V7_INTEGRATION_SPRINT_5B_AUTONOMOUS_OPPORTUNITY_ACTION_PLATFORM',
    version:'v7.0-integration-sprint-5b',status:'PASSED',testsRun:tests.length,tests:tests,generatedAt:new Date().toISOString()};
